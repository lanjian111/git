...
{
    // 回调里只做轻量标记，避免在中断上下文执行耗时解析逻辑。
    // data/length参数在当前实现中不直接使用，因为实际字节读取统一在HMI_LinkTask中完成。
    (void)data;
    (void)length;
    g_hmi_rx_pending = 1;
}
...
void HMI_LinkTask(void)
{
    uint8 cmd_buf[CMD_MAX_SIZE];
    qsize cmd_len = 0;
    ...
    // 本轮开始处理，先清标志；若处理期间又收到新数据，回调会再次置位。
    g_hmi_rx_pending = 0;
