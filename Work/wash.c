...
static uint8_t g_stage1_fill_timeout_active = 0;                     // 第一段进水超时监测是否激活
...
void Wash_ResetState(void)                                           // 复位整个清洗流程状态
{
	...
	g_stage1_fill_timeout_active = 0;                                  // 关闭第一段进水超时监测
...
void Wash_Task(void)                                                 // 每100ms调用一次的清洗状态机任务
{
	...
	case WASH_STATE_INIT:                                             // 流程初始化状态
		...
		g_stage1_fill_timeout_active = 1;                               // 激活第一段进水超时监测
	...
	case WASH_STATE_STAGE1_FILL:                                      // 第一段进水状态
		...
		if (liquid_level_adc > WASH_LEVEL_PUMP_ON_THRESHOLD)           // 液位高于1000允许开循环泵
		{
			...
			g_stage1_fill_timeout_active = 0;                             // 已达到1000，停止30秒短超时监测
		}
		else if (g_stage1_fill_timeout_active && delay_expired(g_wash_state_start_ms, WASH_FILL_TIMEOUT_MS)) // 30秒未达到1000则超时终止
		{
